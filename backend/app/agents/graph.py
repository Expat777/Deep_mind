...
Реши, можно ли по последнему вопросу (с учётом истории) написать осмысленный SQL.
Правила:
- Будь решительным: если запрос в целом понятен — выбирай "sql", даже если формулировка
  не идеальна. Выбери разумную интерпретацию вместо уточнения.
- Просьба построить график/диаграмму — это НЕ повод для уточнения: график строится
  автоматически отдельным инструментом. НИКОГДА не спрашивай про тип графика, формат
  вывода («изображение или данные») и т.п.
- Уточняй ("clarify") ТОЛЬКО если непонятно, КАКИЕ данные нужны (метрика/разрез/период)
  и без этого SQL действительно не написать.
Формат — ТОЛЬКО JSON: {{"route": "sql"}} или
{{"route": "clarify", "question": "<короткий вопрос по сути данных>"}}."""
...
def _wants_plot(state: GraphState) -> bool:
    """Эвристика: просил ли пользователь график в ЛЮБОЙ реплике сессии
    (в диалоге с уточнениями ключевое слово может быть в раннем сообщении)."""
    text = " ".join(
        m.content.lower() for m in state["messages"] if isinstance(m, HumanMessage)
    )
